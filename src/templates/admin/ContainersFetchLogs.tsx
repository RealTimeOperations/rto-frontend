...
export default function ContainersFetchLogs() {
  ...
  // ✅ Log file har load par REPLACE hoti hai (rewrite model — accumulate nahi)
  const load = useCallback(async () => {
    try {
      const res = await fetch(`${API}/containers/logs`)
      if (res.ok) {
        setBackendOk(true)
  ...
  // ✅ 2 second poll — start/stop FORAN reflect ho
  useEffect(() => {
  ...
  async function start() {
    ...
    setMsg(null)
    try {
      const res = await fetch(`${API}/containers/start`, { method: 'POST' })
      const d = await res.json()
      setMsg({ type: d.ok ? 'success' : 'error', text: d.message || (d.ok ? 'Server started' : 'Start failed - check backend') })
      ...
      setMsg({ type: 'error', text: 'Backend respond nahi kar raha (port 8000)' })
    } finally {
      setBusy(null)
    }
  }
  ...
  async function stop() {
    ...
    setMsg(null)
    try {
      const res = await fetch(`${API}/containers/stop`, { method: 'POST' })
      const d = await res.json()
      setMsg({ type: d.ok ? 'success' : 'error', text: d.message || (d.ok ? 'Server stopped' : 'Stop failed - check backend') })
      if (msgTimer.current) window.clearTimeout(msgTimer.current)
      msgTimer.current = window.setTimeout(() => setMsg(null), 6000)
      load()
      setTimeout(load, 800)
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Title + subtitle */}
      <div>
        <h1 className="text-2xl sm:text-3xl font-extrabold text-emerald-300">Containers Fetch Logs</h1>
        <p className="text-xs sm:text-sm text-white/50 mt-1">
          Live output of the containers fetching process (checks the portal every 15 seconds)
        </p>
      </div>
      ...
      {backendOk === false && (
        <div className="px-4 py-3 rounded-xl border bg-red-500/15 border-red-400/40 text-red-300 text-xs sm:text-sm font-semibold">
          ⚠ Backend endpoint /containers/logs not responding — restart backend (python main.py) and make sure containers_router is registered.
        </div>
      )}
      ...
      <div className="rounded-2xl border border-emerald-400/20 bg-[#04231c] overflow-hidden">
        <div className="flex items-center justify-between px-5 py-3 border-b border-emerald-400/15">
          <span className="text-emerald-300 font-extrabold tracking-[0.2em] text-xs sm:text-sm uppercase">
            Containers Fetching Logs
          </span>
          <span className="text-[10px] text-white/40">
            containers_auto.log • {running ? `RUNNING${pid ? ` (PID ${pid})` : ''}` : 'STOPPED'}
          </span>
        </div>
        <div
          ref={boxRef}
        ...
        >
          {lines.length === 0 ? (
            <span className="text-white/40">
              {running ? 'Logs load ho rahe hain…' : 'Server stopped hai — Start Process dabayein.'}
            </span>
          ) : (
            lines.map((ln, i) => (
