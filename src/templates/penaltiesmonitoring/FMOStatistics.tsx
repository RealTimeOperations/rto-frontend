...
export default function FMOStatistics({ penalties, loading = false, permissions }: Props) {
  ...
  // ✅ 4. Imposed Report button sirf admin ya dono offices walon ko dikhe
  const showImposedReport = !permissions || allowedOffices.length === 2
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {imposedOpen && (
        <>
          <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm" onClick={() => setImposedOpen(false)} />
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
            <div className="pointer-events-auto w-full max-w-7xl max-h-[92vh] overflow-y-auto rounded-2xl border border-emerald-400/30 bg-[#04231c] shadow-[0_30px_80px_rgba(0,0,0,0.6)] [scrollbar-width:thin] [scrollbar-color:rgba(16,185,129,0.4)_transparent]">
              ...
              <div className="p-5 flex flex-col gap-3">
                {imposedMatrix.subs.length === 0 ? (
                  <div className="rounded-xl border border-white/10 bg-white/5 p-8 text-center text-xs text-white/40">No TM imposed penalties found for {dateLabel}</div>
                ) : (
                  <div ref={reportRef} className="rounded-xl border border-white/10 bg-[#06281f] p-2 sm:p-3 flex flex-col gap-2">
                    <div className="flex flex-wrap items-center justify-between gap-2 px-1">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <div className="text-[9px] sm:text-xs font-bold text-emerald-300 whitespace-nowrap">• {dateLabel}</div>
                      </div>
